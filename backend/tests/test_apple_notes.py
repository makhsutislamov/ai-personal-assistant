...
def _build_search_output(total_found: int, notes: list[dict]) -> str:
    ...
    for n in notes:
        record = _FIELD_SEP.join(
            [n["note_id"], n["title"], n["body"], n["folder"], n["modified"]]
        )
        records.append(record)
    return _RECORD_SEP.join(records) + _RECORD_SEP


def _build_read_output(title: str, body: str, folder: str = "Notes", modified: str = "Sunday, April 6, 2026") -> str:
...
class TestAppleNotesSearchAgent:
    ...
    async def test_search_success_returns_notes(self, agent):
        notes = [
            {"note_id": "x-coredata://uuid/ICNote/p1", "title": "Meeting Notes", "body": "Discussed Q2 goals.", "folder": "Work", "modified": "Monday, April 1, 2026"},
        ]
        proc = _make_proc(stdout=_build_search_output(1, notes))

        with patch("app.agents.apple_notes._run_applescript_sync", return_value=(proc.stdout, proc.stderr, proc.returncode)):
    ...
    async def test_search_snippet_truncated(self, agent):
        long_body = "A" * 600
        notes = [
            {"note_id": "x-coredata://uuid/ICNote/p2", "title": "Long Note", "body": long_body, "folder": "Notes", "modified": "Monday, April 1, 2026"},
        ]
        proc = _make_proc(stdout=_build_search_output(1, notes))
    ...
    async def test_search_snippet_not_truncated_when_short(self, agent):
        ...
        notes = [
            {"note_id": "x-coredata://uuid/ICNote/p3", "title": "Short Note", "body": short_body, "folder": "Notes", "modified": "Monday, April 1, 2026"},
        ]
        proc = _make_proc(stdout=_build_search_output(1, notes))
    ...
    async def test_search_max_results_cap(self):
        ...
        notes = [
            {"note_id": f"x-coredata://uuid/ICNote/p{i}", "title": f"Note {i}", "body": "content", "folder": "Notes", "modified": "Monday, April 1, 2026"}
            for i in range(3)
        ]
    ...
    async def test_search_permission_denied(self, agent):
        proc = _make_proc(returncode=1, stderr="Notes got an error: not authorized to send Apple events")
...
class TestAppleNotesReadAgent:
    ...
    async def test_read_note_not_found(self, agent):
        proc = _make_proc(returncode=1, stderr="Notes got an error: Can't get note id \"x-coredata://bad\".")
